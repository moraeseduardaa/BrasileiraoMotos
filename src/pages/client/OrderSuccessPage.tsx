...
import {
  ...
  ArrowRight,
  Wallet,
  DollarSign, // Substituir Cash por DollarSign
} from "lucide-react";
...
const OrderSuccessPage = () => {
  ...
  useEffect(() => {
    // Se não houver um orderId, redirecionamos para a página inicial
    // Isso evita acesso direto a esta página
    if (!orderId) {
      navigate("/");
    }
  }, [orderId, navigate]);
  ...
  const getPaymentIcon = () => {
    switch (paymentMethod) {
      ...
      case "bank_slip":
        return <DollarSign className="h-6 w-6" />; // Substituir Cash por DollarSign
      default:
